...
async def run_download_pipeline():
    ...
    async with httpx.AsyncClient(base_url=settings.TARGET_API_URL, timeout=60.0) as client:
        try:
            while True:
                ...
                for i in range(0, len(file_names), chunk_size):
                    ...
                    download_state["status_message"] = f"Скачивание {len(chunk)} файлов..."
                    ...
                    with zipfile.ZipFile(zip_data) as zf:
                        async with aiosqlite.connect(settings.DB_PATH) as db:
                            for fname in zf.namelist():
                                file_content = zf.read(fname).decode("utf-8", errors="ignore")
                                ...
                                download_dt = datetime.now(nsk_tz).strftime("%Y-%m-%d %H:%M:%S")
                    ...
                    download_state["status_message"] = "Подтверждение скачанных файлов..."
                    await api_client.safe_request(
                        client, "POST", "/api/files/downloaded", json={"file_names": chunk}
                    )

        except Exception as e:
            download_state["status_message"] = f"Процесс остановлен из-за ошибки: {str(e)}"
        finally:
